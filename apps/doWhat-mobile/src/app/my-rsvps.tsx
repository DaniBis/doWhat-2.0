...
export default function MyRsvps() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (e2) {
          console.error("Sessions fetch error:", e2);
          setError(e2.message);
        }
        ...
        console.error("My RSVPs error:", e);
        setError(e.message || "Failed to load your RSVPs");
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function updateStatus(activity_id: string, next: Row['status']) {
    try {
      ...
      const uid = auth?.user?.id;
      if (!uid) {
        setError('Please sign in to update your RSVP.');
        return;
      }
