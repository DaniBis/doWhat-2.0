...
describe('OnboardingNavPill', () => {
  ...
  it('renders the floating CTA when steps remain', async () => {
    __supabaseMock.setTraitCount(2);

    render(<OnboardingNavPill />);
    await act(async () => {});

    await waitFor(() => expect(screen.getByTestId('onboarding-nav-pill-cta')).toBeTruthy());
    expect(screen.getByText(/Finish onboarding/i)).toBeTruthy();
    expect(screen.getByText(/Next: Pick 5 base traits/i)).toBeTruthy();

    fireEvent.press(screen.getByTestId('onboarding-nav-pill-cta'));
  ...
  it('hides itself once onboarding is complete', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('onboarding-nav-pill-cta')).toBeNull();
    });
  });
});
