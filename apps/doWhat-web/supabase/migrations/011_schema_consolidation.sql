...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='sessions') THEN
    IF NOT has_created_by THEN
      RAISE NOTICE '[events] sessions.created_by missing; skipping event backfill';
    ELSE
      ...
      EXECUTE format(
        'INSERT INTO public.events(host_id, starts_at, ends_at, status, source_session_id)
         ...
         WHERE e.id IS NULL AND s.created_by IS NOT NULL',
         's.created_by',
    ...
    EXECUTE format(
      'INSERT INTO public.event_participants(event_id, user_id, role, rsvp_status, updated_at)
       ...
       WHERE ep.user_id IS NULL',
        CASE WHEN has_status THEN ' (CASE r.status WHEN ''going'' THEN ''going'' WHEN ''interested'' THEN ''maybe'' ELSE ''declined'' END)::public.rsvp_status '
          ELSE '''going''::public.rsvp_status' END
    );
  END IF;
