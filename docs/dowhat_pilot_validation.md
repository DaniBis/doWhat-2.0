...
export DOWHAT_SEED_PASSWORD="doWhat-Pilot-Password!"
...
1. Ensures the three Bucharest hosts exist in Supabase auth:
   - `mara.padel.host@dowhat.dev` (padel)
   - `alex.run.host@dowhat.dev` (running)
   - `ioana.climb.host@dowhat.dev` (climbing)
2. Upserts their `profiles`, `user_sport_profiles`, availability windows, and accepts the reliability pledge (version `dowhat-v1`).
3. Upserts venues + activities tagged with `dowhat:bucuresti` metadata.
...
The script reuses deterministic UUIDs, so rerunning it is idempotent. Newly created users (when `DOWHAT_SEED_PASSWORD` is not set) are logged in the terminal with their generated password.

Run `pnpm verify:dowhat` (same Supabase env vars) to confirm the host profiles, sport profiles, venues, activities, sessions, open slots, and host attendance rows are all present before moving on. If you need to start over, `pnpm rollback:dowhat` removes every seeded session/slot/venue/activity/profile/auth user so the next seed run starts from a clean slate.
...
This checks env wiring, required migrations (`--dowhat` roster), trait policies, verifies the Bucharest pilot entities via `scripts/verify-dowhat.mjs`, and pings the `/api/health` endpoint.
...
- Double-check that `pnpm seed:dowhat` succeeded (no Supabase errors) and that your mobile session is authenticated (otherwise the open-slot fetch falls back).
...
- **Missing hosts or sessions:** rerun `pnpm seed:dowhat`. Because IDs are deterministic, the upserts will repair partial data without duplicating records. If you need a full reset, run `pnpm rollback:dowhat` before reseeding.
...
- **Wrong password:** set `DOWHAT_SEED_PASSWORD` and rerun the seed. The script will update auth users with the new password.
...
- **Removing pilot data:** delete rows with the `dowhat:bucuresti` seed tag from `venues`/`activities`/`sessions` and revoke the pilot users in Supabase auth. There isn’t an automated cleaner yet (file an issue if needed).
