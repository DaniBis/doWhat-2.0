...
- `scripts/seed-dowhat.mjs`, new `scripts/dowhat-shared.mjs`, `scripts/rollback-dowhat.mjs` — doWhat pilot tooling refactor.
...
| doWhat pilot seeding | `scripts/seed-dowhat.mjs`, `scripts/dowhat-shared.mjs`, `scripts/rollback-dowhat.mjs`, README/docs | Complete & documented | Verify new rollback script in staging; ensure `pnpm health` references stay consistent. |
...
1. **Playwright server contention / env drift:** Runner now scopes to the 10 admin e2e specs, injects Supabase env defaults, defaults to port 4302, and the `/admin/new` open-slot spec intercepts `/sessions/session-e2e-open` so deleting sessions during the run no longer spams server fetch errors. `pnpm --filter dowhat-web exec playwright test` will still fail if another process already occupies 4302; free the port or export a different `PLAYWRIGHT_PORT` before rerunning e2e tests. As of **19 Dec 2025** the repo ships `.env.playwright.example` plus automatic loading inside `playwright.config.ts`; copy it to `.env.playwright.local` and set `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and `NEXT_PUBLIC_ADMIN_EMAILS` before running the suite.
...
- `pnpm verify:dowhat` (sanity-checks the Bucharest pilot data and matches the health script expectations)
...
2. **Decide on Expo Doctor warning** — either delete `ios/`/`android/` folders (managed workflow) or document manual config syncing before cutting an EAS build. (Status: managed-only workflow already adopted; reran Expo Doctor on 14 Dec and captured the clean run, so no immediate work required unless native folders return.)
...
4. **Monitor Find a 4th inventory** — keep the doWhat seed fresh (`pnpm seed:dowhat && pnpm verify:dowhat`) so the ranked hero carousel always has open slots to highlight. Latest reseed + verify executed on **19 Dec 2025** against project `kdviydoftmjuglaglsmm` after replaying migrations `035/038/043/044`; see `ASSISTANT_CHANGES_LOG.md` for the log snapshot.
